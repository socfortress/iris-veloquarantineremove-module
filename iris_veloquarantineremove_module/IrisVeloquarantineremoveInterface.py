    ...
    def register_hooks(self, module_id: int):
        ...
        if module_conf.get('veloquarantineremove_manual_hook_enabled'):
            status = self.register_to_hook(module_id, iris_hook_name='on_manual_trigger_asset',
                                           manual_hook_name='Remove Quarantine')
    ...
    def _handle_asset(self, data) -> InterfaceStatus.IIStatus:
        ...
        for asset in data:
            ...
            if 'Linux -' in asset.asset_type.asset_name:
                status = veloquarantineremove_handler.handle_linux(asset=asset)
                in_status = InterfaceStatus.merge_status(in_status, status)

            if 'Windows -' in asset.asset_type.asset_name:
                status = veloquarantineremove_handler.handle_windows(asset=asset)
                in_status = InterfaceStatus.merge_status(in_status, status)
            ...
            # elif element.ioc_type.type_name in etc...
                ...
                self.log.error(f'IOC type {asset.asset_type.asset_name} not handled by veloquarantineremove module. Skipping')
