...
import iris_veloquarantineremove_module.IrisVeloquarantineremoveConfig as interface_conf
from iris_veloquarantineremove_module.veloquarantineremove_handler.veloquarantineremove_handler import VeloquarantineremoveHandler


class IrisVeloquarantineremoveInterface(IrisModuleInterface):
    """
    Provide the interface between Iris and veloquarantineremoveHandler
    """
    name = "IrisVeloquarantineremoveInterface"
    ...
    def register_hooks(self, module_id: int):
        ...
        if module_conf.get('veloquarantineremove_on_create_hook_enabled'):
        ...
        if module_conf.get('veloquarantineremove_on_update_hook_enabled'):
        ...
        if module_conf.get('veloquarantineremove_manual_hook_enabled'):
            status = self.register_to_hook(module_id, iris_hook_name='on_manual_trigger_ioc',
                                           manual_hook_name='Get veloquarantineremove insight')
            if status.is_failure():
                self.log.error(status.get_message())
                self.log.error(status.get_data())
    ...
    def _handle_ioc(self, data) -> InterfaceStatus.IIStatus:
        ...
        veloquarantineremove_handler = VeloquarantineremoveHandler(mod_config=self.module_dict_conf,
        ...
        for element in data:
            ...
            if 'domain' in element.ioc_type.type_name:
                status = veloquarantineremove_handler.handle_domain(ioc=element)
                in_status = InterfaceStatus.merge_status(in_status, status)

            #elif element.ioc_type.type_name in ['md5', 'sha224', 'sha256', 'sha512']:
            #    status = veloquarantineremove_handler.handle_hash(ioc=element)
            #    in_status = InterfaceStatus.merge_status(in_status, status)
            #
            # elif element.ioc_type.type_name in etc...

            else:
                self.log.error(f'IOC type {element.ioc_type.type_name} not handled by veloquarantineremove module. Skipping')
